...
fn main() {
    ...
    let answer: usize = page_updates.iter_mut().filter_map(|update| {
        ...
        break Some(update[update.len() / 2])
    }).sum();
    // println!("{:?}", page_updates);
...
mod test {
    ...
    fn first_test() {
        ...
        println!("{:?}", ordering_rules);
        println!("{:?}", page_updates);
        let answer: usize = page_updates.iter_mut().filter_map(|mut update| {
            for rule in ordering_rules.iter() {
                let first = update.iter().find_position(|e| **e == rule.0);
                let second = update.iter().find_position(|e| **e == rule.1);
